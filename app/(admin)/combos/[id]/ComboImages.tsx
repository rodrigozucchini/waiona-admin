...
import {
  ...
  updateComboImage,
  uploadComboImage,
} from '@/actions/combo-image.actions'
...
export function ComboImages({ comboId, images }: ComboImagesProps) {
  ...
  function handlePositionChange(id: number, newPosition: string) {
    startTransition(async () => {
      const result = await updateComboImage(id, { position: Number(newPosition) })
      if (!result.success) {
        toast.error(result.message)
        return
      }
  ...
  function handleDelete(id: number) {
    ...
    startTransition(async () => {
      const result = await deleteComboImage(id)
      if (!result.success) {
        toast.error(result.message)
        return
      }
