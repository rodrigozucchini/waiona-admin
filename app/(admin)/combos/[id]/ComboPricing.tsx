...
interface ComboPricingProps {
  comboId: number
  pricing: ComboPricingResponseDto | null
}
...
export function ComboPricing({ comboId, pricing }: ComboPricingProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = pricing
        ? await updateComboPricing(pricing.id, payload)
        : await createComboPricing({ comboId, ...payload })
  ...
  function handleDelete() {
    if (!pricing || !confirm('¿Borrar el precio de este combo?')) return
    startTransition(async () => {
