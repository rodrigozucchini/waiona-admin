...
import { addCouponComboTarget, removeCouponComboTarget } from '@/actions/coupon-target.actions'
import type { ComboResponseDto, CouponComboTargetResponseDto } from '@/core/types'

interface CouponComboTargetsProps {
  couponId: number
  targets: CouponComboTargetResponseDto[]
  combos: ComboResponseDto[]
}

export function CouponComboTargets({ couponId, targets, combos }: CouponComboTargetsProps) {
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await addCouponComboTarget(couponId, Number(comboId))
      if (!result.success) {
  ...
  function handleRemove(cId: number) {
    if (!confirm('¿Quitar este combo del cupón?')) return
    startTransition(async () => {
      const result = await removeCouponComboTarget(couponId, cId)
      if (!result.success) {
        toast.error(result.message)
        return
      }
